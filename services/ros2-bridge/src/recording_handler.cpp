...
void RecordingHandler::convertTopicStatus(
    ...
    switch (ros_topic_status.rate_status) {
        ...
        case proto_recorder_msgs::msg::TopicStatus::RATE_STATUS_NO_MESSAGES:
            proto_topic_status->set_rate_status(drs::ros2bridge::v1::TopicStatus::RATE_STATUS_NO_MESSAGES);
            break;
        default:
...
bool RecordingHandler::matchesRecordingFilter(const std::string& hardware_id, const std::string& filter) {
...
bool RecordingHandler::matchesTopicFilter(const std::string& topic_name, const std::string& filter) {
